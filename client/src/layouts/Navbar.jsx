    ...
    <header className="sticky top-0 z-30 flex h-16 items-center gap-4 border-b border-surface-border bg-surface-card/80 px-4 backdrop-blur lg:px-6">
      ...
      <div className="ml-auto flex items-center gap-2">
            ...
            <div className="absolute right-0 mt-2 w-80 rounded-xl border border-surface-border bg-surface-card shadow-pop">
              ...
              <ul className="max-h-72 overflow-y-auto">
                {notifications.map((n) => (
                  <li
                    key={n.id}
                    className="flex cursor-pointer items-start gap-3 border-b border-surface-border px-4 py-3 last:border-0 hover:bg-surface"
                    onClick={() => showToast(n.title, 'info')}
                  >
                    <span className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${n.unread ? 'bg-accent-500' : 'bg-transparent'}`} />
                    <div>
              ...
              <button
                type="button"
                onClick={() => { setNotifOpen(false); showToast('All notifications marked as read', 'success') }}
                className="w-full rounded-b-xl px-4 py-2.5 text-center text-xs font-medium text-accent-600 hover:bg-surface"
              >
        ...
        <div className="relative" ref={profileRef}>
          ...
          >
            <div className="flex h-8 w-8 items-center justify-center rounded-full bg-navy-900 text-xs font-semibold text-white">SV</div>
          ...
          {profileOpen && (
            <div className="absolute right-0 mt-2 w-56 rounded-xl border border-surface-border bg-surface-card shadow-pop py-1.5">
              ...
              <button type="button" onClick={() => { setProfileOpen(false); showToast('Profile page coming soon', 'info') }} className="flex w-full items-center gap-2.5 px-3.5 py-2 text-sm text-ink-700 hover:bg-surface">
                <UserCircle className="h-4 w-4 text-ink-400" /> My Profile
              </button>
              <button type="button" onClick={() => { setProfileOpen(false); showToast('Settings page coming soon', 'info') }} className="flex w-full items-center gap-2.5 px-3.5 py-2 text-sm text-ink-700 hover:bg-surface">
                <Settings className="h-4 w-4 text-ink-400" /> Settings
              </button>
              <div className="my-1 border-t border-surface-border" />
              <button type="button" onClick={() => { localStorage.removeItem('token'); setProfileOpen(false); showToast('Signed out', 'success') }} className="flex w-full items-center gap-2.5 px-3.5 py-2 text-sm text-rose-600 hover:bg-rose-50">
